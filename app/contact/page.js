...
function Contact(){
  ...
  const { smallscreen, LScreen, SScreen } = hooks();
   ...
   <div style={{backgroundImage: "url('/bgs/waterfall.webp')",  backgroundSize: "cover", backgroundPosition: "center", color:'white'}}>
     {LScreen && <div className={`flex items-center justify-center min-h-screen`}>
        ...
        style={{ backgroundColor: "rgba(0, 0, 0, 0.4)", backdropFilter: "blur(2px)"}}>
            <form className="w-[80%] flex flex-col p-[4px]">
                <div className=''>
                    <input  type="text" placeholder='NAME' 
                     className="border border-gray-400 p-2 h-[5vh] w-[30vw] rounded" />
                    <input type="email" placeholder='Email'
                     className="border border-gray-400 p-2 h-[5vh] w-[30vw] rounded"/>
                    <textarea placeholder="Your message" 
                      className="border border-gray-400 p-2 rounded resize-none h-[10vh] w-[30vw]"/> 
                </div> 
            </form>
        </div>
     </div>}
